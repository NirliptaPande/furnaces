...
def rfreg(eco):
    ...
    np.save('../../output/zeros_y_pred_%s.npy'%str(eco),y_pred)
...
echoes_test = x_test[:,[6,14,16,19,27,34,35,37]].copy()
...
#testing the model with all the variables
x_train = np.delete(x_train,[0,1,2,6,14,15,16,18,19,21,22,23,24,25,26,27,32,34,35,36,37],axis = 1)#Train with the economic factors
model = RandomForestRegressor(n_estimators=500, criterion='squared_error', random_state=42,
                                    n_jobs=60, min_samples_split= 5,bootstrap=True,
                                max_features = 7, oob_score=True)
# Note: I am aware that max_features shows an error, but it's not, it runs perfectly fine, it is supposed to accept int
...
y_pred = model.predict(x_test)
np.save('../../output/zeros_y_pred_none.npy',y_pred)
"""
...
for i in range(echoes.shape[1]):
    rfreg(i)
echoes_test = np.delete(echoes_test,[4],axis = 1)
echoes = np.delete(echoes,[4],axis = 1)
model = RandomForestRegressor(n_estimators=500, criterion='squared_error', random_state=42,
                                    n_jobs=60, min_samples_split= 5,bootstrap=True,
                                max_features = 7, oob_score=True)
y_pred = model.predict(x_test)
np.save('../../output/zeros_y_pred_all.npy',y_pred)